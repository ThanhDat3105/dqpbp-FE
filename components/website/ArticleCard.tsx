...
export default function ArticleCard({
  ...
  if (variant === "vertical") {
    return (
      <div className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow border border-gray-100 flex flex-col">
        <div className="bg-[#546a2f]/10 h-40 flex items-center justify-center text-[#546a2f]/30 relative">
          {article.thumbnail ? (
            <Image
        ...
        <div className="p-4 flex flex-col flex-1">
          <span
            className={`text-xs font-medium px-2 py-0.5 rounded-full w-fit mb-2 ${badgeClass}`}
          >
            {article.category}
          </span>
  ...
  return (
    <div className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow border border-gray-100 flex gap-4 p-4">
      {/* Thumbnail */}
      <div className="bg-[#546a2f]/10 rounded-lg w-24 h-20 shrink-0 flex items-center justify-center text-[#546a2f]/30 relative">
        {article.thumbnail ? (
          <Image
            src={article.thumbnail}
            alt={article.title}
