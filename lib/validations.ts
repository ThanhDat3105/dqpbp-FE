...
const reportFieldSchema = z.object({
  name: z.string().min(1, { message: "Tên hạng mục báo cáo là bắt buộc" }),
...
const taskSchema = z.object({
  ...
  report_fields: z.array(reportFieldSchema).default([]),
...
export const createActivitySchema = z
  .object({
    ...
    end_date: z.string().min(1, { message: "Ngày kết thúc là bắt buộc" }),
    tasks: z.array(taskSchema).default([]),
    created_by: z.string().min(1, { message: "Người tạo là bắt buộc" }),
    created_at: z.string().min(1, { message: "Ngày tạo là bắt buộc" }),
    updated_at: z.string().min(1, { message: "Ngày cập nhật là bắt buộc" }),
  })
  .refine(
    (data) => {
      const startDate =
